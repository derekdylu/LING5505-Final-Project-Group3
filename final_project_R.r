...
text <- list.files(path = "./", full.names = FALSE)## AER¡Ñ
paragraph_num <- vector("numeric", le?gth = length(text))## number of paragraph
...
for(i in seq_along(text)){
  post[[i]] <- r?adLines(text[i], encoding = "UTF-8", warn = FALSE)
...
for(i in seq?along(post)){ 
  sentence_num[i] <- length(unlist(strsplit(post[i], split = "[¡C¡I¡H]", fixed = FALSE)))
}
